...
import { runEeLaptopSync, EeLaptopSyncMode } from "../src/server/eeLaptops/sync";
...
async function main() {
  ...
  const report = await runEeLaptopSync(args);
...
function modeArg(value: string | boolean | undefined): EeLaptopSyncMode {
  const mode = typeof value === "string" ? value : "prices";
  if (["discover", "full", "prices", "validate", "promote"].includes(mode)) return mode as EeLaptopSyncMode;
