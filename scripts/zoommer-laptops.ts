...
import { runZoommerLaptopSync, ZoommerLaptopSyncMode } from "../src/server/zoommerLaptops/sync";
...
async function main() {
  ...
  const report = await runZoommerLaptopSync(args);
...
function modeArg(value: string | boolean | undefined): ZoommerLaptopSyncMode {
  const mode = typeof value === "string" ? value : "prices";
  if (["discover", "full", "prices", "validate", "promote"].includes(mode)) return mode as ZoommerLaptopSyncMode;
