...
import { runZoommerConsoleSync, ZoommerConsoleSyncMode } from "../src/server/zoommerConsoles/sync";
...
async function main() {
  ...
  const report = await runZoommerConsoleSync(args);
...
function modeArg(value: string | boolean | undefined): ZoommerConsoleSyncMode {
  const mode = typeof value === "string" ? value : "prices";
  if (["discover", "full", "prices", "validate", "promote"].includes(mode)) return mode as ZoommerConsoleSyncMode;
