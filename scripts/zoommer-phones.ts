...
import { runZoommerPhoneSync, ZoommerPhoneSyncMode } from "../src/server/zoommerPhones/sync";
...
async function main() {
  ...
  const report = await runZoommerPhoneSync(args);
...
function modeArg(value: string | boolean | undefined): ZoommerPhoneSyncMode {
  const mode = typeof value === "string" ? value : "prices";
  if (["discover", "full", "prices", "validate", "promote"].includes(mode)) return mode as ZoommerPhoneSyncMode;
