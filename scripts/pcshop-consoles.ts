...
import { runPcshopConsoleSync, PcshopConsoleSyncMode } from "../src/server/pcshopConsoles/sync";
...
async function main() {
  ...
  const report = await runPcshopConsoleSync(args);
...
function modeArg(value: string | boolean | undefined): PcshopConsoleSyncMode {
  const mode = typeof value === "string" ? value : "prices";
  if (["discover", "full", "prices", "validate", "promote"].includes(mode)) return mode as PcshopConsoleSyncMode;
