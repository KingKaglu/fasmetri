...
const CATEGORY_ID = 688;
const CATEGORY_URL = "https://zoommer.ge/en/playstation-c688";
...
const ADVISORY_LOCK_ID = 90000000002;
...
export type ZoommerConsoleSyncMode = "discover" | "full" | "prices" | "validate" | "promote";

export type ZoommerConsoleSyncOptions = {
  mode: ZoommerConsoleSyncMode;
  promote?: boolean;
  rawFile?: string;
  reportDir?: string;
  rawDir?: string;
...
export type StagedZoommerConsole = {
  store: typeof STORE;
...
type ValidationReport = {
  ...
  oldActiveZoommerConsoleCount: number;
  newScrapedZoommerConsoleCount: number;
...
export type ZoommerConsoleSyncReport = {
  startedAt: string;
  finishedAt: string;
  durationMs: number;
  mode: ZoommerConsoleSyncMode;
...
export async function runZoommerConsoleSync(options: ZoommerConsoleSyncOptions): Promise<ZoommerConsoleSyncReport> {
  ...
  return withSyncLock(async () => {
    ...
    const report: ZoommerConsoleSyncReport = {
    ...
    if (validation.hardFailures.length && (options.promote || options.mode === "promote")) {
      throw new Error(`Zoommer console sync validation failed: ${validation.hardFailures.join("; ")}`);
    }
    return report;
  });
}

async function scrapeSnapshot(options: ZoommerConsoleSyncOptions): Promise<ZoommerConsolesSnapshot> {
  ...
  const failedDetailUrls: ZoommerConsolesSnapshot["failedDetailUrls"] = [];
  ...
  const snapshot: ZoommerConsolesSnapshot = {
    version: 1,
    mode,
...
function stagedFromListing(product: ZoommerListingProduct, listingPage: number): StagedZoommerConsole {
  ...
  return {
    ...
    // Listing API returns isInStock=false for every product (unreliable stub); real stock from detail page.
...
async function scrapeDetail(item: StagedZoommerConsole) {
  try {
    ...
    return {
      ...
      // Detail page carries the real stock flag.
      availability:
...
async function validateSnapshot(snapshot: ZoommerConsolesSnapshot): Promise<ValidationReport> {
  ...
  return {
    ...
    oldActiveZoommerConsoleCount,
    newScrapedZoommerConsoleCount: productCount,
...
async function promoteSnapshot(snapshot: ZoommerConsolesSnapshot): Promise<PromotionReport> {
  ...
  for (const item of snapshot.products) {
    ...
    if (priceAnomalous) {
      priceAnomalyCount += 1;
      console.warn(
        `[zoommer-consoles] Price anomaly: "${item.originalTitle}" ${existingPrice} -> ${currentPriceGel} GEL, keeping old price (${item.productUrl})`,
      );
    ...
    // Detail page is the only reliable stock source; when UNKNOWN keep existing.
...
async function upsertStandaloneProduct(tx: Prisma.TransactionClient, categoryId: string, item: StagedZoommerConsole, existingOfferId?: string) {
  if (existingOfferId) {
...
function productData(categoryId: string, item: StagedZoommerConsole) {
  return {
    ...
    categoryReason: "Zoommer PlayStation-category sync validated this source URL.",
    categoryMatchedRules: jsonValue(["zoommer-playstation-category-688"]),
...
async function upsertRawOffer(shopId: string, item: StagedZoommerConsole, tx?: Prisma.TransactionClient) {
  ...
  const data = {
    ...
    importBatchId: `zoommer-consoles:${item.scrapedAt.slice(0, 10)}`,
    storeKey: STORE,
    brand: item.normalizedSpecs.brand ?? item.brand,
    model: item.normalizedSpecs.model,
      ...
      await db.rawOffer.update({
        ...
        data: {
          ...
          errorMessage: inactive
            ? `Not present in ${INACTIVE_MISS_THRESHOLD} consecutive Zoommer PlayStation syncs.`
            : "Possibly removed: not present in latest Zoommer PlayStation listing.",
          processedAt: now,
        },
      });
    }
...
async function assertSyncSchemaReady() {
  ...
  try {
    ...
    if (isMissingColumnError(error)) {
      throw new Error("Zoommer console sync schema is not deployed. Run npm run db:deploy before promotion.");
    }
    throw error;
  }
}
...
function normalizeConsoleSpecs(input: {
  ...
  brand = brand ?? stringValue(spec(["Brand", "ბრენდი"])) ?? stringValue((input.listing as ZoommerListingProduct)?.brandName);

  // Storage
  const storageGb = normalizeStorageGb(spec(["Internal memory", "Storage", "ინტერნალ მეხსიერება"])) ?? normalizeStorageGb(title);

  // Edition/version
  const descText = stringValue((input.detailProduct as JsonRecord | undefined)?.short_description) ?? "";
  ...
  // Color from spec or title
  const color = spec(["Color", "ფერი"]) ?? extractColorFromTitle(title);

  return { brand, model, version, storageGb, color };
}

function normalizeSnapshot(snapshot: ZoommerConsolesSnapshot): ZoommerConsolesSnapshot {
  return {
    ...snapshot,
    products: snapshot.products.map((product) => ({
      ...product,
      normalizedSpecs: mergeDefinedConsoleSpecs(
        product.normalizedSpecs,
        normalizeConsoleSpecs({
...
function mergeDefinedConsoleSpecs(base: NormalizedConsoleSpecs, supplement: NormalizedConsoleSpecs): NormalizedConsoleSpecs {
  const merged: NormalizedConsoleSpecs = { ...base };
  for (const [key, value] of Object.entries(supplement) as Array<[keyof NormalizedConsoleSpecs, NormalizedConsoleSpecs[keyof NormalizedConsoleSpecs]]>) {
    if (value !== undefined && value !== null && value !== "") {
      (merged as Record<keyof NormalizedConsoleSpecs, NormalizedConsoleSpecs[keyof NormalizedConsoleSpecs]>)[key] = value;
    }
  }
  return merged;
}
...
function rawSpecsPayload(item: StagedZoommerConsole) {
  return {
    source: SOURCE,
    sourceCategory: SOURCE_CATEGORY,
...
function productIdentityFor(item: StagedZoommerConsole) {
  return {
...
function isPlayStationListing(product: StagedZoommerConsole) {
...
function stableProductSlug(item: StagedZoommerConsole) {
...
function readSnapshot(rawFile?: string): ZoommerConsolesSnapshot {
  const file = rawFile ? resolve(rawFile) : resolve(defaultRawDir(), "zoommer-consoles-sync-latest.json");
  if (!existsSync(file)) throw new Error(`Raw snapshot does not exist: ${file}`);
  return JSON.parse(readFileSync(file, "utf8")) as ZoommerConsolesSnapshot;
}

function writeSnapshot(snapshot: ZoommerConsolesSnapshot, rawDir?: string) {
  const dir = writableDir(rawDir ?? defaultRawDir());
  const timestamp = timestampForFile(new Date());
  const file = join(dir, `zoommer-consoles-sync-${timestamp}.json`);
  const latest = join(dir, "zoommer-consoles-sync-latest.json");
...
function writeReport(report: ZoommerConsoleSyncReport, reportDir?: string) {
  const dir = writableDir(reportDir ?? defaultReportDir());
  const timestamp = timestampForFile(new Date(report.finishedAt));
  const reportFile = join(dir, `zoommer-consoles-sync-${timestamp}.json`);
  const latestReportFile = join(dir, "zoommer-consoles-sync-latest.json");
...
function defaultRawDir() {
  return join(".codex-logs", "zoommer-consoles", "raw");
}
...
function writableDir(preferred: string) {
  try {
    ...
    const fallback = join(tmpdir(), "fasmetri-zoommer-console-sync", preferred.replace(/[:\\/]+/g, "-"));
...
async function withSyncLock<T>(callback: () => Promise<T>) {
  const lockPath = join(".codex-logs", "zoommer-consoles-sync.lock");
  ...
  try {
    if (db) {
      ...
      if (!dbLocked) throw new Error("Another Zoommer console sync is already running.");
    }
...
function acquireFileLock(path: string) {
  ...
  if (existsSync(path)) {
    ...
    if (Number.isFinite(startedAt) && Date.now() - startedAt < 6 * 60 * 60 * 1000) {
      throw new Error("Another Zoommer console sync lock is active.");
    }
