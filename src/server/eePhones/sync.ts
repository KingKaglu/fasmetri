...
const SOURCE_CATEGORY = "mobile-phones";
const FASMETRI_CATEGORY = "mobiles";
const CATEGORY_ID = 377;
const CATEGORY_URL = "https://ee.ge/en/mobile-phone-c377t";
...
export type EePhoneSyncOptions = {
...
export async function runEePhoneSync(options: EePhoneSyncOptions): Promise<EePhoneSyncReport> {
  ...
  return withSyncLock(async () => {
    ...
    const report: EePhoneSyncReport = {
    ...
    if (validation.hardFailures.length && (options.promote || options.mode === "promote")) {
      throw new Error(`EE phone sync validation failed: ${validation.hardFailures.join("; ")}`);
    }
    return report;
  });
}

async function scrapeSnapshot(options: EePhoneSyncOptions): Promise<EePhonesSnapshot> {
  ...
  const failedDetailUrls: EePhonesSnapshot["failedDetailUrls"] = [];
  ...
  const snapshot: EePhonesSnapshot = {
    version: 1,
    mode,
    ...
    listing: {
      ...
      totalUniquePhoneUrls: listing.products.length,
...
async function discoverListing() {
  ...
  const unique = new Map<string, StagedEePhone>();
...
function stagedFromListing(product: EEListingProduct, listingPage: number): StagedEePhone {
  const route = routeForProduct(product);
...
async function scrapeDetail(item: StagedEePhone) {
  try {
...
function mergeDetail(item: StagedEePhone, detail?: Awaited<ReturnType<typeof scrapeDetail>>, existing?: ExistingRawState): StagedEePhone {
  if (detail) {
    return {
      ...item,
...
async function validateSnapshot(snapshot: EePhonesSnapshot): Promise<ValidationReport> {
  ...
  for (const product of snapshot.products) {
    ...
    if (!isMobilePhoneListing(product)) nonPhoneProductCount += 1;
  ...
  const oldActiveEePhoneUrls = await activeEePhoneUrls();
  const oldActiveEePhoneCount = oldActiveEePhoneUrls.length;
  const hardFailures: string[] = [];
  const warnings: string[] = [];
  const productCount = snapshot.products.length;

  if (nonPhoneProductCount) hardFailures.push(`${nonPhoneProductCount} non-phone products detected.`);
  ...
  if (oldActiveEePhoneCount > 0 && productCount > 0 && productCount < Math.floor(oldActiveEePhoneCount * LOW_COUNT_RATIO)) {
    ...
    const missing = oldActiveEePhoneUrls.filter((url) => !scrapedKeys.has(url.replace(/\/$/, "")));
    const probe = await probeDelisting(missing, { userAgent: USER_AGENT, timeoutMs: REQUEST_TIMEOUT_MS });
    const line = `New scraped count ${productCount} is suspiciously lower than old active count ${oldActiveEePhoneCount}`;
  ...
  return {
    ...
    missingStorageCount,
    missingRamCount,
    nonPhoneProductCount,
    invalidStoreCount,
    invalidSourceCount,
    invalidCategoryCount,
    invalidEEUrlCount,
    duplicateUniqueKeyCount,
    oldActiveEePhoneCount,
    newScrapedEePhoneCount: productCount,
...
async function promoteSnapshot(snapshot: EePhonesSnapshot): Promise<PromotionReport> {
  ...
  const category = await db.category.upsert({
    ...
    update: {},
    create: { slug: FASMETRI_CATEGORY, nameKa: "Phones", nameEn: "Phones" },
  });
...
async function upsertStandaloneProduct(tx: Prisma.TransactionClient, categoryId: string, item: StagedEePhone, existingOfferId?: string) {
  if (existingOfferId) {
...
function productData(categoryId: string, item: StagedEePhone) {
  return {
    ...
    categoryReason: "EE mobile-phone-only sync validated this source URL.",
    categoryMatchedRules: jsonValue(["ee-mobile-phones-category-377"]),
...
async function upsertRawOffer(shopId: string, item: StagedEePhone, tx?: Prisma.TransactionClient) {
  if (!db) throw new Error("DATABASE_URL is required for promotion.");
  const client = tx ?? db;
  ...
  const data = {
    ...
    importBatchId: `ee-phones:${item.scrapedAt.slice(0, 10)}`,
    storeKey: STORE,
    brand: item.normalizedSpecs.brand ?? item.brand,
    model: item.normalizedSpecs.normalizedModelName,
      ...
      await db.rawOffer.update({
        ...
        data: {
          ...
          errorMessage: inactive
            ? `Not present in ${INACTIVE_MISS_THRESHOLD} consecutive EE mobile-phone syncs.`
            : "Possibly removed: not present in latest EE mobile-phone listing.",
          processedAt: now,
        },
      });
    }
...
async function activeEePhoneUrls(): Promise<string[]> {
  if (!db) return [];
  const where = {
    shop: { slug: STORE },
    product: {
...
async function assertSyncSchemaReady() {
  ...
  try {
    ...
    if (isMissingColumnError(error)) {
      throw new Error("EE phone sync schema is not deployed. Run npm run db:deploy before promotion.");
    }
    throw error;
  }
}
...
function normalizePhoneSpecs(input: {
  title: string;
  ...
  const simType = spec(["SIM", "SIM card"]) ?? detectSimType(title);
  const internalMemory = spec(["Memory", "Internal memory", "Internal Memory"]);
  const ram = spec(["RAM"]);
  const displayFeature = specInGroup("Display", ["Additional feature", "Additional feature "]);
  const screenSize = spec(["Display size", "Screen size", "Screen Size"]);
  const refreshRate = spec(["Refresh rate", "Refresh Rate"]) ?? displayFeature;
  const battery = spec(["Battery type/size", "Battery"]);
  const chargingSpeed = spec(["Charging", "Charging Speed", "Charging speed"]);
  const mainCamera = specInGroup("Main Camera", ["camera", "Main camera", " Main camera"]);
  const selfieCamera = spec(["Selfie Camera", "Front camera", " Front camera"]);
  const system = [spec(["OS", "System"]), spec(["System Version"])].filter(Boolean).join(" ").trim() || undefined;
  const technicalFeature = specInGroup("Technical Specifications", ["Additional feature", "Additional feature "]);
  const releaseYear = parseYear(spec(["Announced", "Release date", "Release Date"]));

  return {
    brand,
    model: extractModelName(title, brand, color),
    normalizedModelName: normalizeModelName(title, brand, color),
    ...
    fiveGSupport: detect5g(title, spec(["5G", "Technology"])),
    screenSize,
    screenType: spec(["Display Type", "Screen type", "Screen Type"]),
    ...
    ipProtection: spec(["IP Protection"]) ?? extractIpProtection(technicalFeature),
...
function extractIpProtection(value?: string) {
  if (!value || !/\bIP\d{2}[A-Z]?\b/i.test(value)) return undefined;
  return value.trim();
}
...
function rawSpecsPayload(item: StagedEePhone) {
  return {
    source: SOURCE,
    sourceCategory: SOURCE_CATEGORY,
...
function productIdentityFor(item: StagedEePhone) {
  return {
    ...
    simType: item.normalizedSpecs.simType,
    esimSupport: item.normalizedSpecs.esimSupport,
    fiveGSupport: item.normalizedSpecs.fiveGSupport,
    canonicalKey: item.uniqueKey,
    normalizedTitle: normalizeProductTitle(item.originalTitle),
    cleanTitle: removeNoiseWords(item.originalTitle),
...
function isMobilePhoneListing(product: StagedEePhone) {
  const pathname = new URL(product.productUrl).pathname;
  const routeOk = /^\/en\/mobile-phone\/[^/]+-p\d+\/?$/i.test(pathname);
  const outlet = /\/(autleti|outlet)\//i.test(pathname);
  ...
  return routeOk && !outlet && categoryId === String(CATEGORY_ID) && parentCategory.includes("mobile") && categoryName.includes("phone");
}
...
function normalizeStorageGb(value?: string) {
  ...
  const combinedWithUnits = value.match(/\b\d{1,2}\s*gb\s*\/\s*(\d{2,4})\s*gb\b/i);
  if (combinedWithUnits) return Number(combinedWithUnits[1]);
...
function normalizeBrand(value?: string) {
  ...
  if (lower === "poco" || lower === "redmi") return "Xiaomi";
  const known = PHONE_BRANDS.find((item) => item.toLowerCase() === lower);
  return known ?? brand;
}
...
function extractBrand(title: string) {
  ...
  return PHONE_BRANDS.find((brand) => lower.includes(brand.toLowerCase()));
}
...
function extractModelName(title: string, brand?: string, color?: string) {
  ...
  value = value
    .replace(/\b\d{1,2}\s*gb\s*\/\s*\d{2,4}\s*gb\b/gi, " ")
    .replace(/\b\d{1,2}\s*\/\s*\d{2,4}\s*gb\b/gi, " ")
    .replace(/\b\d{2,4}\s*gb\b/gi, " ")
    .replace(/\b\d+(?:\.\d+)?\s*tb\b/gi, " ")
    .replace(/\be-?sim\s+only\b/gi, " ")
    .replace(/\bsim\s*\+\s*e-?sim\b/gi, " ")
    .replace(/\bsim\s*\+\s*e\s+sim\b/gi, " ")
    .replace(/\be-?sim\b/gi, " ")
    .replace(/\be\s+sim\b/gi, " ")
...
function extractSeries(title: string) {
  ...
  if (normalized.includes("redmi")) return "Redmi";
  if (normalized.includes("poco")) return "Poco";
  if (normalized.includes("razr")) return "RAZR";
  return undefined;
}
...
function detectSimType(title: string) {
  if (/\be[\s-]?sim\s+only\b/i.test(title)) return "e-SIM Only";
  if (/\bsim\s*\+\s*e[\s-]?sim\b/i.test(title)) return "SIM + eSIM";
  if (/\be[\s-]?sim\b/i.test(title)) return "eSIM";
  return undefined;
}

function detectEsim(title: string, simType?: string, specValueText?: string) {
  if (/\be[\s-]?sim\b/i.test(title)) return true;
  if (simType && /e[\s-]?sim/i.test(simType)) return true;
  return booleanSpec(specValueText);
}

function detect5g(title: string, specValueText?: string) {
  if (/\b5g\b/i.test(title)) return true;
  if (specValueText && /\b5g\b/i.test(specValueText)) return true;
  return booleanSpec(specValueText);
}
...
function parseYear(value?: string) {
  ...
  return year ? Number(year) : parseInteger(value);
}
...
function stableProductSlug(item: StagedEePhone) {
...
function readSnapshot(rawFile?: string): EePhonesSnapshot {
  const file = rawFile ? resolve(rawFile) : resolve(defaultRawDir(), "ee-phones-sync-latest.json");
  if (!existsSync(file)) throw new Error(`Raw snapshot does not exist: ${file}`);
  return JSON.parse(readFileSync(file, "utf8")) as EePhonesSnapshot;
}

function writeSnapshot(snapshot: EePhonesSnapshot, rawDir?: string) {
  const dir = writableDir(rawDir ?? defaultRawDir());
  const timestamp = timestampForFile(new Date());
  const file = join(dir, `ee-phones-sync-${timestamp}.json`);
  const latest = join(dir, "ee-phones-sync-latest.json");
...
function writeReport(report: EePhoneSyncReport, reportDir?: string) {
  const dir = writableDir(reportDir ?? defaultReportDir());
  const timestamp = timestampForFile(new Date(report.finishedAt));
  const reportFile = join(dir, `ee-phones-sync-${timestamp}.json`);
  const latestReportFile = join(dir, "ee-phones-sync-latest.json");
...
function defaultRawDir() {
  return join(".codex-logs", "ee-phones", "raw");
}
...
function writableDir(preferred: string) {
  try {
    ...
    const fallback = join(tmpdir(), "fasmetri-ee-phone-sync", preferred.replace(/[:\\/]+/g, "-"));
...
async function withSyncLock<T>(callback: () => Promise<T>) {
  const lockPath = join(".codex-logs", "ee-phones-sync.lock");
  ...
  try {
    if (db) {
      ...
      if (!dbLocked) throw new Error("Another EE mobile-phone sync is already running.");
    }
...
function acquireFileLock(path: string) {
  ...
  if (existsSync(path)) {
    ...
    if (Number.isFinite(startedAt) && Date.now() - startedAt < 6 * 60 * 60 * 1000) {
      throw new Error("Another EE mobile-phone sync lock is active.");
    }
