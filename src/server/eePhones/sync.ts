...
async function validateSnapshot(snapshot: EePhonesSnapshot): Promise<ValidationReport> {
  ...
  const oldActiveEePhoneCount = await activeEePhoneCount();
  ...
  if (oldActiveEePhoneCount > 0 && productCount < Math.floor(oldActiveEePhoneCount * LOW_COUNT_RATIO)) {
    hardFailures.push(`New scraped count ${productCount} is suspiciously lower than old active count ${oldActiveEePhoneCount}.`);
  }
...
async function activeEePhoneCount() {
  if (!db) return 0;
  const where = {
    shop: { slug: STORE },
    product: {
