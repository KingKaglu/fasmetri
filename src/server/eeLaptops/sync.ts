...
const CATEGORY_URL = "https://ee.ge/en/laptops-c58t";
...
const ADVISORY_LOCK_ID = 5820260605;

type JsonRecord = Record<string, unknown>;

export type EeLaptopSyncMode = "discover" | "full" | "prices" | "validate" | "promote";

export type EeLaptopSyncOptions = {
  mode: EeLaptopSyncMode;
  promote?: boolean;
  rawFile?: string;
  reportDir?: string;
  rawDir?: string;
...
type ValidationReport = {
  ...
  oldActiveEeLaptopCount: number;
  newScrapedEeLaptopCount: number;
...
export type EeLaptopSyncReport = {
  startedAt: string;
  finishedAt: string;
  durationMs: number;
  mode: EeLaptopSyncMode;
...
export async function runEeLaptopSync(options: EeLaptopSyncOptions): Promise<EeLaptopSyncReport> {
  ...
  return withSyncLock(async () => {
    ...
    const report: EeLaptopSyncReport = {
      ...
      listing: {
        firstBatchCount: snapshot.listing.firstBatchCount,
        pagesLoaded: snapshot.listing.totalBatchesLoaded,
        uniqueLaptopUrls: snapshot.listing.totalUniqueLaptopUrls,
        duplicateUrls: snapshot.listing.duplicateUrlCount,
        failedListingUrls: snapshot.listing.failedListingUrls,
      },
      validation,
      promotion,
    };
    ...
    if (validation.hardFailures.length && (options.promote || options.mode === "promote")) {
      throw new Error(`EE laptop sync validation failed: ${validation.hardFailures.join("; ")}`);
    }
    return report;
  });
}

async function scrapeSnapshot(options: EeLaptopSyncOptions): Promise<EeLaptopsSnapshot> {
  ...
  const failedDetailUrls: EeLaptopsSnapshot["failedDetailUrls"] = [];
  ...
  const snapshot: EeLaptopsSnapshot = {
    version: 1,
    mode,
...
async function discoverListing() {
  ...
  const unique = new Map<string, StagedEeLaptop>();
...
function stagedFromListing(product: EEListingProduct, listingPage: number): StagedEeLaptop {
  const route = routeForProduct(product);
...
async function scrapeDetail(item: StagedEeLaptop) {
  try {
...
function mergeDetail(item: StagedEeLaptop, detail?: Awaited<ReturnType<typeof scrapeDetail>>, existing?: ExistingRawState): StagedEeLaptop {
  if (detail) {
    return {
      ...item,
...
function normalizeSnapshot(snapshot: EeLaptopsSnapshot): EeLaptopsSnapshot {
  return {
    ...snapshot,
    products: snapshot.products.map((product) => {
      const normalizedSpecs = mergeDefinedLaptopSpecs(
        product.normalizedSpecs,
        normalizeLaptopSpecs({
          title: product.originalTitle,
          listing: product.rawListingData,
          rawSpecValues: rawSpecValuesFromRawSpecs(product.rawSpecs),
        }),
      );
      return {
        ...product,
        brand: normalizedSpecs.brand ?? product.brand,
        normalizedSpecs,
      };
    }),
  };
...
function mergeDefinedLaptopSpecs(base: NormalizedLaptopSpecs, next: NormalizedLaptopSpecs): NormalizedLaptopSpecs {
  ...
  return merged as NormalizedLaptopSpecs;
}

function rawSpecValuesFromRawSpecs(rawSpecs: JsonRecord): EESpecValue[] {
  const flattened = Array.isArray(rawSpecs.flattened) ? rawSpecs.flattened : [];
  const values: EESpecValue[] = [];
  for (const item of flattened) {
    ...
    if (!specificationName || !specificationMeaning || specificationMeaning === "-") continue;
    values.push({
      groupName,
      specificationName,
...
async function validateSnapshot(snapshot: EeLaptopsSnapshot): Promise<ValidationReport> {
  ...
  const oldActiveEeLaptopUrls = await activeEeLaptopUrls();
  const oldActiveEeLaptopCount = oldActiveEeLaptopUrls.length;
  ...
  if (!snapshot.listing.seeMoreExhausted) hardFailures.push("EE listing pages were not fully exhausted.");
  if (oldActiveEeLaptopCount > 0 && productCount > 0 && productCount < Math.floor(oldActiveEeLaptopCount * LOW_COUNT_RATIO)) {
    // The count collapsed. Ask EE whether the offers we are missing are
    ...
    const missing = oldActiveEeLaptopUrls.filter((url) => !scrapedKeys.has(url.replace(/\/$/, "")));
    const probe = await probeDelisting(missing, { userAgent: USER_AGENT, timeoutMs: REQUEST_TIMEOUT_MS });
    const line = `New scraped count ${productCount} is suspiciously lower than old active count ${oldActiveEeLaptopCount}`;
    if (probe.delistingConfirmed) {
      warnings.push(`${line} — EE delisted them (${probe.summary}); promoting so the stale offers deactivate.`);
    } else {
      hardFailures.push(
  ...
  return {
    ...
    oldActiveEeLaptopCount,
    newScrapedEeLaptopCount: productCount,
...
async function promoteSnapshot(snapshot: EeLaptopsSnapshot): Promise<PromotionReport> {
  ...
  for (const item of snapshot.products) {
    ...
    if (priceAnomalous) {
      priceAnomalyCount += 1;
      console.warn(
        `[ee-laptops] Price anomaly: "${item.originalTitle}" ${existingPrice} -> ${currentPriceGel} GEL, keeping old price (${item.productUrl})`,
      );
...
async function upsertStandaloneProduct(tx: Prisma.TransactionClient, categoryId: string, item: StagedEeLaptop, existingOfferId?: string) {
  if (existingOfferId) {
...
function productData(categoryId: string, item: StagedEeLaptop) {
  return {
    ...
    categoryReason: "Elite Electronics laptop-only sync validated this source URL.",
    categoryMatchedRules: jsonValue(["ee-laptops-category-58"]),
...
async function upsertRawOffer(shopId: string, item: StagedEeLaptop, tx?: Prisma.TransactionClient) {
  if (!db) throw new Error("DATABASE_URL is required for promotion.");
  const client = tx ?? db;
      ...
      await db.rawOffer.update({
        ...
        data: {
          ...
          errorMessage: inactive
            ? `Not present in ${INACTIVE_MISS_THRESHOLD} consecutive EE laptop syncs.`
            : "Possibly removed: not present in latest EE laptop listing.",
          processedAt: now,
        },
      });
    }
...
async function activeEeLaptopUrls(): Promise<string[]> {
  if (!db) return [];
  const where = {
    shop: { slug: STORE },
    product: {
...
async function assertSyncSchemaReady() {
  ...
  try {
    ...
    if (isMissingColumnError(error)) {
      throw new Error("EE laptop sync schema is not deployed. Run npm run db:deploy before promotion.");
    }
    throw error;
  }
}
...
function normalizeLaptopSpecs(input: {
  ...
  const specInGroup = (group: string, names: string[]) => specValueInGroup(input.rawSpecValues, group, names);
  ...
  const series = spec(["Series"]) ?? extractSeries(title);
  const model = spec(["Model", "მოდელი"]) ?? series ?? extractModelName(title, brand, color);
  const modelCode = extractModelCode(title);
  const processorBrand = specInGroup("General", ["Processor Brand"]);
  const processorModel = specInGroup("Processor", ["Processor Model"]) ?? spec(["Central processor", "Processor", "Processor Generation", "ცენტრალური პროცესორი", "პროცესორის თაობა"]);
  const processorModelNumber = specInGroup("Technical Specifications", ["Processor Model Number"]);
  const cpu = normalizeCpu(combineCpu(processorBrand, processorModel, processorModelNumber) ?? title);
  const gpu = normalizeGpu(
    specInGroup("Technical Specifications", ["Graphic Processor", "Graphics processor"]) ??
      spec(["Graphics processor", "Graphic Processor", "გრაფიკული პროცესორი"]) ??
      title,
  );
  const cpuParts = splitCpu(cpu);
  const gpuParts = splitGpu(gpu);
  const internalMemory = specInGroup("Memory", ["SSD"]) ?? specInGroup("Memory", ["HDD"]) ?? spec(["Storage", "Internal memory", "Internal Memory", "შიდა მეხსიერება"]);
  const internalMemoryType =
    specInGroup("Memory", ["SSD Type", "HDD Type"]) ??
    spec(["Internal Memory Type", "Internal Memory interface", "შიდა მეხსიერების ტიპი", "შიდა მეხსიერების ინტერფეისი"]);
  ...
  const wifi = specInGroup("Technical Specifications", ["WLAN Standards"]) ?? spec(["Wi-Fi", "WiFi", "Wireless", "Wireless internet"]);
  const ports = collectPorts(spec);
  const webcam = specInGroup("Features", ["Built-in Webcam Resolutuion", "Built-in Webcam Resolution"]) ?? spec(["Web Camera", "Webcam", "ვებ კამერა"]);
  ...
  return {
    ...
    modelCode,
    laptopType: specInGroup("General", ["Type", "Category"]) ?? spec(["Laptop Type", "ლეპტოპის ტიპი"]),
    series,
    ...
    ramGb: normalizeRamGb(ram) ?? normalizeRamGb(ramTypeValue) ?? normalizeRamGb(title),
    ramType: normalizeRamType(ramTypeValue) ?? normalizeRamType(ramSpeed) ?? normalizeRamType(title),
    color,
    cpu,
    cpuBrand: cpuParts.brand,
    cpuModel: cpuParts.model,
    cpuGeneration: cpuParts.generation,
    gpu,
    gpuBrand: gpuParts.brand,
    gpuModel: gpuParts.model,
    gpuVramGb: gpuParts.vramGb,
    screenSize,
    screenResolution,
    screenType: specInGroup("Display", ["Display Type", "Display Technology"]) ?? spec(["Screen type", "Screen Type", "ეკრანის ტიპი"]),
    refreshRateHz: parseInteger(refreshRate),
    touchscreen: booleanSpec(specInGroup("Display", ["Touch Screen", "Touchscreen"]) ?? spec(["Touch Screen", "Touchscreen", "სენსორული ეკრანი"])),
    operatingSystem: specInGroup("General", ["Operating System"]) ?? spec(["Operating system", "Operating System", "ოპერაციული სისტემა"]),
    battery: specInGroup("Battery", ["Battery Power"]) ?? spec(["Battery", "Element type", "ბატარეა"]),
    weightKg: parseWeightKg(specInGroup("Dimensions", ["Weight"]) ?? spec(["Weight", "წონა"])),
    ...
    webcam: webcam ? booleanSpec(webcam) ?? true : undefined,
    warranty: spec(["Warranty"]) ?? stringValue(input.detailProduct?.fullWarranty) ?? stringValue(input.detailProduct?.firstTierWarranty),
  };
}
...
function specValueInGroup(values: EESpecValue[], groupName: string, names: string[]) {
  const normalizedGroup = normalizeSpecKey(groupName);
  const normalizedNames = new Set(names.map((name) => normalizeSpecKey(name)));
  return values.find(
    (value) => normalizeSpecKey(value.groupName) === normalizedGroup && normalizedNames.has(normalizeSpecKey(value.specificationName)),
  )?.specificationMeaning;
}
...
function productIdentityFor(item: StagedEeLaptop) {
  return {
    productType: "laptop",
    categorySlug: FASMETRI_CATEGORY,
    sourceCategory: SOURCE_CATEGORY,
    ...
    cpuBrand: item.normalizedSpecs.cpuBrand,
    cpuModel: item.normalizedSpecs.cpuModel,
    cpuGeneration: item.normalizedSpecs.cpuGeneration,
    gpu: item.normalizedSpecs.gpu,
    gpuBrand: item.normalizedSpecs.gpuBrand,
    gpuModel: item.normalizedSpecs.gpuModel,
    gpuVramGb: item.normalizedSpecs.gpuVramGb,
...
function isLaptopListing(product: StagedEeLaptop) {
  const routeOk = /^\/en\/laptops\/[^/]+-p\d+\/?$/i.test(new URL(product.productUrl).pathname);
  const listing = product.rawListingData;
  const categoryId = String(listing.categoryId ?? "");
  const categoryName = String(listing.categoryName ?? "").toLowerCase();
  return routeOk && categoryId === String(CATEGORY_ID) && categoryName.includes("laptop");
}
...
function normalizeStorageGb(value?: string) {
  ...
  const bareStorage = value.trim().match(/^(\d{2,4})$/);
  if (bareStorage) return Number(bareStorage[1]);
  return undefined;
}
...
function normalizeRamGb(value?: string) {
  ...
  const combined = value.match(/\b(\d{1,2})\s*\/\s*\d{2,4}\s*gb\b/i);
  if (combined) return Number(combined[1]);
  const sum = value.match(/\b(\d{1,2})\s*\+\s*(\d{1,2})\s*gb\b/i);
  ...
  const bareRam = value.trim().match(/^(\d{1,2})$/);
  return bareRam ? Number(bareRam[1]) : undefined;
}
...
function combineCpu(brand?: string, model?: string, modelNumber?: string) {
  const cleanModel = cleanSpecText(model);
  if (!cleanModel) return undefined;
  const cleanBrand = cleanSpecText(brand);
  const cleanNumber = cleanSpecText(modelNumber);
  const withBrand =
    cleanBrand && !new RegExp(`^${escapeRegExp(cleanBrand)}\\b`, "i").test(cleanModel)
      ? `${cleanBrand} ${cleanModel}`
      : cleanModel;
  if (!cleanNumber) return withBrand;
  return /\b\d{3,5}[A-Z]*\b/i.test(withBrand) ? withBrand : `${withBrand} ${cleanNumber}`;
}
...
function splitCpu(cpu?: string) {
  const clean = cleanSpecText(cpu);
  if (!clean) return {};
  const lower = clean.toLowerCase();
  const brand = lower.includes("apple")
    ? "Apple"
    : lower.includes("snapdragon") || lower.includes("qualcomm")
      ? "Qualcomm"
      : lower.includes("amd") || lower.includes("ryzen")
        ? "AMD"
        : lower.includes("intel") || lower.includes("core")
          ? "Intel"
          : undefined;
  return {
    brand,
    model: clean,
    generation: cpuGeneration(clean, brand),
  };
}

function cpuGeneration(cpu: string, brand?: string) {
  if (brand === "Apple") return cpu.match(/\bM\d(?:\s*(?:Pro|Max|Ultra))?\b/i)?.[0];
  if (brand === "Qualcomm") return cpu.match(/\bX\s*(?:Elite|Plus)?\b/i)?.[0]?.replace(/\s+/g, " ");
  if (brand === "AMD") {
    const number = cpu.match(/\bRyzen\s+[3579]\s+(\d{4,5})/i)?.[1];
    return number ? `${number[0]}000-series` : undefined;
  }
  if (brand === "Intel") {
    const number = cpu.match(/\b(?:i[3579]|Ultra\s+[3579]|Core\s+[3579])[-\s]+(\d{3,5})/i)?.[1];
    if (!number) return undefined;
    if (number.length >= 4 && /^1[0-4]/.test(number)) return `${number.slice(0, 2)}th Gen`;
    return `${number[0]}th Gen`;
  }
  return undefined;
}

function splitGpu(gpu?: string) {
  const clean = cleanSpecText(gpu);
  if (!clean) return {};
  const lower = clean.toLowerCase();
  const brand = lower.includes("nvidia") || lower.includes("geforce") || lower.includes("rtx") || lower.includes("gtx")
    ? "NVIDIA"
    : lower.includes("amd") || lower.includes("radeon")
      ? "AMD"
      : lower.includes("intel") || lower.includes("iris") || lower.includes("arc") || lower.includes("uhd")
        ? "Intel"
        : lower.includes("apple") || lower.includes("core gpu")
          ? "Apple"
          : undefined;
  return {
    brand,
    model: clean,
    vramGb: parseGpuVramGb(clean),
  };
}

function parseGpuVramGb(value?: string) {
  if (!value) return undefined;
  const match = value.match(/\b(\d{1,2})\s*gb\b/i);
...
function stableProductSlug(item: StagedEeLaptop) {
...
function readSnapshot(rawFile?: string): EeLaptopsSnapshot {
  const file = rawFile ? resolve(rawFile) : resolve(defaultRawDir(), "ee-laptops-sync-latest.json");
  if (!existsSync(file)) throw new Error(`Raw snapshot does not exist: ${file}`);
  return JSON.parse(readFileSync(file, "utf8")) as EeLaptopsSnapshot;
}

function writeSnapshot(snapshot: EeLaptopsSnapshot, rawDir?: string) {
  const dir = writableDir(rawDir ?? defaultRawDir());
  const timestamp = timestampForFile(new Date());
  const file = join(dir, `ee-laptops-sync-${timestamp}.json`);
  const latest = join(dir, "ee-laptops-sync-latest.json");
...
function writeReport(report: EeLaptopSyncReport, reportDir?: string) {
  const dir = writableDir(reportDir ?? defaultReportDir());
  const timestamp = timestampForFile(new Date(report.finishedAt));
  const reportFile = join(dir, `ee-laptops-sync-${timestamp}.json`);
  const latestReportFile = join(dir, "ee-laptops-sync-latest.json");
...
function defaultRawDir() {
  return join(".codex-logs", "ee-laptops", "raw");
}
...
function writableDir(preferred: string) {
  try {
    ...
    const fallback = join(tmpdir(), "fasmetri-ee-laptop-sync", preferred.replace(/[:\\/]+/g, "-"));
...
async function withSyncLock<T>(callback: () => Promise<T>) {
  const lockPath = join(".codex-logs", "ee-laptops-sync.lock");
  ...
  try {
    if (db) {
      ...
      if (!dbLocked) throw new Error("Another EE laptop sync is already running.");
    }
...
function acquireFileLock(path: string) {
  ...
  if (existsSync(path)) {
    ...
    if (Number.isFinite(startedAt) && Date.now() - startedAt < 6 * 60 * 60 * 1000) {
      throw new Error("Another EE laptop sync lock is active.");
    }
