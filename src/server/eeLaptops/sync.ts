...
async function validateSnapshot(snapshot: EeLaptopsSnapshot): Promise<ValidationReport> {
  ...
  const oldActiveEeLaptopCount = await activeEeLaptopCount();
  ...
  if (oldActiveEeLaptopCount > 0 && productCount < Math.floor(oldActiveEeLaptopCount * LOW_COUNT_RATIO)) {
    hardFailures.push(`New scraped count ${productCount} is suspiciously lower than old active count ${oldActiveEeLaptopCount}.`);
  }
...
async function activeEeLaptopCount() {
  if (!db) return 0;
  const where = {
    shop: { slug: STORE },
    product: {
