...
async function validateSnapshot(snapshot: ZoommerPhonesSnapshot): Promise<ValidationReport> {
  ...
  const oldActiveZoommerPhoneCount = await activeZoommerPhoneCount();
  ...
  if (oldActiveZoommerPhoneCount > 0 && productCount < Math.floor(oldActiveZoommerPhoneCount * LOW_COUNT_RATIO)) {
    hardFailures.push(`New scraped count ${productCount} is suspiciously lower than old active count ${oldActiveZoommerPhoneCount}.`);
  }
...
async function activeZoommerPhoneCount() {
  if (!db) return 0;
  const where = {
    shop: { slug: STORE },
    product: {
