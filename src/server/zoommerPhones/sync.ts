...
const CATEGORY_ID = 855;
const CATEGORY_URL = "https://zoommer.ge/en/mobile-phones-c855";
...
export type ZoommerPhoneSyncMode = "discover" | "full" | "prices" | "validate" | "promote";

export type ZoommerPhoneSyncOptions = {
  mode: ZoommerPhoneSyncMode;
  promote?: boolean;
  rawFile?: string;
  reportDir?: string;
  rawDir?: string;
...
export type StagedZoommerPhone = {
  store: typeof STORE;
...
type ValidationReport = {
  ...
  oldActiveZoommerPhoneCount: number;
  newScrapedZoommerPhoneCount: number;
...
export type ZoommerPhoneSyncReport = {
  startedAt: string;
  finishedAt: string;
  durationMs: number;
  mode: ZoommerPhoneSyncMode;
...
export async function runZoommerPhoneSync(options: ZoommerPhoneSyncOptions): Promise<ZoommerPhoneSyncReport> {
  ...
  return withSyncLock(async () => {
    ...
    const report: ZoommerPhoneSyncReport = {
    ...
    if (validation.hardFailures.length && (options.promote || options.mode === "promote")) {
      throw new Error(`Zoommer phone sync validation failed: ${validation.hardFailures.join("; ")}`);
    }
    return report;
  });
}

async function scrapeSnapshot(options: ZoommerPhoneSyncOptions): Promise<ZoommerPhonesSnapshot> {
  ...
  const failedDetailUrls: ZoommerPhonesSnapshot["failedDetailUrls"] = [];
  ...
  const snapshot: ZoommerPhonesSnapshot = {
    version: 1,
    mode,
...
async function discoverListing() {
  ...
  const unique = new Map<string, StagedZoommerPhone>();
...
function stagedFromListing(product: ZoommerListingProduct, listingPage: number): StagedZoommerPhone {
...
async function scrapeDetail(item: StagedZoommerPhone) {
  try {
...
function mergeDetail(item: StagedZoommerPhone, detail?: Awaited<ReturnType<typeof scrapeDetail>>, existing?: ExistingRawState): StagedZoommerPhone {
  if (detail) {
    return {
      ...item,
...
async function validateSnapshot(snapshot: ZoommerPhonesSnapshot): Promise<ValidationReport> {
  ...
  const oldActiveZoommerPhoneUrls = await activeZoommerPhoneUrls();
  const oldActiveZoommerPhoneCount = oldActiveZoommerPhoneUrls.length;
  ...
  if (oldActiveZoommerPhoneCount > 0 && productCount > 0 && productCount < Math.floor(oldActiveZoommerPhoneCount * LOW_COUNT_RATIO)) {
    ...
    const missing = oldActiveZoommerPhoneUrls.filter((url) => !scrapedKeys.has(url.replace(/\/$/, "")));
    const probe = await probeDelisting(missing, { userAgent: USER_AGENT, timeoutMs: REQUEST_TIMEOUT_MS });
    const line = `New scraped count ${productCount} is suspiciously lower than old active count ${oldActiveZoommerPhoneCount}`;
  ...
  return {
    ...
    oldActiveZoommerPhoneCount,
    newScrapedZoommerPhoneCount: productCount,
...
async function promoteSnapshot(snapshot: ZoommerPhonesSnapshot): Promise<PromotionReport> {
  ...
  for (const item of snapshot.products) {
    ...
    if (priceAnomalous) {
      priceAnomalyCount += 1;
      console.warn(
        `[zoommer-phones] Price anomaly: "${item.originalTitle}" ${existingPrice} -> ${currentPriceGel} GEL, keeping old price (${item.productUrl})`,
      );
...
async function upsertStandaloneProduct(tx: Prisma.TransactionClient, categoryId: string, item: StagedZoommerPhone, existingOfferId?: string) {
  if (existingOfferId) {
...
function productData(categoryId: string, item: StagedZoommerPhone) {
  return {
    ...
    categoryReason: "Zoommer mobile-phone-only sync validated this source URL.",
    categoryMatchedRules: jsonValue(["zoommer-mobile-phones-category-855"]),
...
async function upsertRawOffer(shopId: string, item: StagedZoommerPhone, tx?: Prisma.TransactionClient) {
  ...
  const data = {
    ...
    importBatchId: `zoommer-phones:${item.scrapedAt.slice(0, 10)}`,
    storeKey: STORE,
    brand: item.normalizedSpecs.brand ?? item.brand,
    model: item.normalizedSpecs.normalizedModelName,
      ...
      await db.rawOffer.update({
        ...
        data: {
          ...
          errorMessage: inactive
            ? `Not present in ${INACTIVE_MISS_THRESHOLD} consecutive Zoommer mobile-phone syncs.`
            : "Possibly removed: not present in latest Zoommer mobile-phone listing.",
          processedAt: now,
        },
      });
    }
...
async function activeZoommerPhoneUrls(): Promise<string[]> {
  if (!db) return [];
  const where = {
    shop: { slug: STORE },
    product: {
...
async function assertSyncSchemaReady() {
  ...
  try {
    ...
    if (isMissingColumnError(error)) {
      throw new Error("Zoommer phone sync schema is not deployed. Run npm run db:deploy before promotion.");
    }
    throw error;
  }
}
...
function rawSpecsPayload(item: StagedZoommerPhone) {
  return {
    source: SOURCE,
    sourceCategory: SOURCE_CATEGORY,
...
function productIdentityFor(item: StagedZoommerPhone) {
  return {
...
function isMobilePhoneListing(product: StagedZoommerPhone) {
  const routeOk = /\/mobile-phones\//i.test(new URL(product.productUrl).pathname);
  ...
  return routeOk && (categoryId === String(CATEGORY_ID) || parentCategory.includes("mobile"));
}
...
function detectSimType(title: string) {
  if (/\be-?sim\s+only\b/i.test(title)) return "e-SIM Only";
  if (/\bsim\s*\+\s*e-?sim\b/i.test(title)) return "SIM + eSIM";
  if (/\be-?sim\b/i.test(title)) return "eSIM";
  return undefined;
}

function detectEsim(title: string, simType?: string, specValueText?: string) {
  if (/\be-?sim\b/i.test(title)) return true;
  if (simType && /e-?sim/i.test(simType)) return true;
...
function stableProductSlug(item: StagedZoommerPhone) {
...
function readSnapshot(rawFile?: string): ZoommerPhonesSnapshot {
  const file = rawFile ? resolve(rawFile) : resolve(defaultRawDir(), "zoommer-phones-sync-latest.json");
  if (!existsSync(file)) throw new Error(`Raw snapshot does not exist: ${file}`);
  return JSON.parse(readFileSync(file, "utf8")) as ZoommerPhonesSnapshot;
}

function writeSnapshot(snapshot: ZoommerPhonesSnapshot, rawDir?: string) {
  const dir = writableDir(rawDir ?? defaultRawDir());
  const timestamp = timestampForFile(new Date());
  const file = join(dir, `zoommer-phones-sync-${timestamp}.json`);
  const latest = join(dir, "zoommer-phones-sync-latest.json");
...
function writeReport(report: ZoommerPhoneSyncReport, reportDir?: string) {
  const dir = writableDir(reportDir ?? defaultReportDir());
  const timestamp = timestampForFile(new Date(report.finishedAt));
  const reportFile = join(dir, `zoommer-phones-sync-${timestamp}.json`);
  const latestReportFile = join(dir, "zoommer-phones-sync-latest.json");
...
function defaultRawDir() {
  return join(".codex-logs", "zoommer-phones", "raw");
}
...
function writableDir(preferred: string) {
  try {
    ...
    const fallback = join(tmpdir(), "fasmetri-zoommer-phone-sync", preferred.replace(/[:\\/]+/g, "-"));
...
async function withSyncLock<T>(callback: () => Promise<T>) {
  const lockPath = join(".codex-logs", "zoommer-phones-sync.lock");
  ...
  try {
    if (db) {
      ...
      if (!dbLocked) throw new Error("Another Zoommer mobile-phone sync is already running.");
    }
...
function acquireFileLock(path: string) {
  ...
  if (existsSync(path)) {
    ...
    if (Number.isFinite(startedAt) && Date.now() - startedAt < 6 * 60 * 60 * 1000) {
      throw new Error("Another Zoommer mobile-phone sync lock is active.");
    }
