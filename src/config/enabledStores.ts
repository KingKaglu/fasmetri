...
export const STORE_CONFIGS: StoreConfig[] = [
  ...
  { key: "isurve", name: "iSurve", baseUrl: "https://isurve.ge", enabled: true, priority: "medium", notes: "Shopify storefront, synced from the standard public Shopify JSON endpoints (npm run scrape:isurve:full) — no HTML scraping, and plain Node fetch gets 200 (not the Alta case: no curl/capitalised-header workaround). /collections/all/products.json looks like a one-call catalogue dump but Shopify hard-caps its cursor — page=100&limit=250 returns rows and page=101 returns HTTP 400, i.e. a 25,000-product ceiling under a catalogue that spans 48 product sub-sitemaps — so the sync is driven per collection via /collections/<handle>/products.json instead. The sitemap index mirrors every sub-sitemap under /en/ and /ru/; only the default Georgian locale is used, and product URLs are flat /products/<handle>. THREE FIELDS ARE POISON, verified live 2026-09-23: variants[].sku is not a SKU but installment marketing copy (\"/ თვეში 47 ლარი\") or empty — 2,500 out of 2,500 sampled rows were one or the other, so it must never reach externalId or model; vendor is not a brand but the warranty badge (\"ხარისხის გარანტია\", \"კომერციული გარანტია 1 წელი\"); barcode is empty everywhere. product_type is mostly a Facebook-feed audience label (\"ქალი\"/\"კაცი\"/\"ორივე\", \"Sareklamo WMN\", \"No Feed\") on 76% of rows — a baking mould and a TP-Link smart bulb share product_type \"ქალი\" — so it is DROPPED unless it matches REAL_PRODUCT_TYPES, because breadcrumbs are a scoring signal for categorizeProduct() and \"ქალი\" is a live beauty keyword. Identity is rebuilt from the explicit \"მოდელი: <code>\" line in body_html (~87% coverage) with the title as the fallback signal, brand from the brand-<Brand> tag, and externalId from the Shopify numeric id (store-local, in-shop dedup only). Titles mix Cyrillic homoglyphs into Latin/digit runs (\"305х76\" uses U+0445, not Latin x), so titles, descriptions and model codes are homoglyph-folded before extraction or dedup would silently fail. Scope: iSurve is a generalist (pools, garden, toys, jewellery, cookware, car chemicals), so only collections whose own Georgian title resolves to a PUBLIC_CATEGORY_SLUG through categorizeProduct() are walked — brand hubs (Bosch/Samsung/Apple, ~1,330 collections total) and price-band/installment/outlet shelves are never drivers. Adapter verified against live endpoints 2026-09-23; discover (319 in-scope collections of ~1,330) and read-only full dry runs passed, and the CLI refuses --promote while this flag is false. Enabled 2026-09-23 for the first real import. A 300-row trial (2026-09-22) wrote RawOffers only (234 NORMALIZED / 66 NEEDS_REVIEW, 0 ProductOffers); the full dry run of 2026-09-24 (317 collections, 7,779 products, 7,821 offers incl. one per Shopify variant) passed with 0 incomplete collections, and nothing beyond that trial has been written." },
